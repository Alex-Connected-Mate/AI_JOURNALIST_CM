...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE question_id = NEW.question_id AND option_id = NEW.choice
  ) INTO result_exists;
  ...
  SELECT COUNT(*) FROM votes WHERE question_id = NEW.question_id INTO total_votes;
...
CREATE POLICY questions_owner_policy ON questions
  USING (EXISTS (SELECT 1 FROM sessions WHERE sessions.id = questions.session_id AND sessions.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM sessions WHERE sessions.id = questions.session_id AND sessions.user_id = auth.uid()));
...
CREATE POLICY vote_results_owner_policy ON vote_results
  ...
  USING (EXISTS (
    ...
    WHERE questions.id = vote_results.question_id AND sessions.user_id = auth.uid()
  ));
...
CREATE POLICY vote_results_participant_policy ON vote_results
  ...
  USING (EXISTS (
    SELECT 1 FROM questions 
    WHERE questions.id = vote_results.question_id AND questions.is_active = true
  ));
