...
CREATE INDEX IF NOT EXISTS idx_participant_votes_deleted_at ON participant_votes(deleted_at) WHERE deleted_at IS NULL;
...
-- Index pour exclure les enregistrements supprimés
CREATE INDEX IF NOT EXISTS idx_vote_settings_deleted_at ON vote_settings(deleted_at) WHERE deleted_at IS NULL;

-- Table: résultats des votes par session
CREATE TABLE IF NOT EXISTS participant_vote_results (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  session_id UUID REFERENCES sessions(id) ON DELETE CASCADE NOT NULL,
...
-- Index pour accélérer les recherches
CREATE INDEX IF NOT EXISTS idx_participant_vote_results_session_id ON participant_vote_results(session_id);
CREATE INDEX IF NOT EXISTS idx_participant_vote_results_participant_id ON participant_vote_results(participant_id);
CREATE INDEX IF NOT EXISTS idx_participant_vote_results_is_top_voted ON participant_vote_results(is_top_voted);
-- Index pour exclure les enregistrements supprimés
CREATE INDEX IF NOT EXISTS idx_participant_vote_results_deleted_at ON participant_vote_results(deleted_at) WHERE deleted_at IS NULL;
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM participant_vote_results 
    WHERE session_id = NEW.session_id AND participant_id = NEW.voted_for_id AND deleted_at IS NULL
  ) INTO result_exists;
  
  -- Si l'enregistrement n'existe pas, le créer
  IF NOT result_exists THEN
    INSERT INTO participant_vote_results (session_id, participant_id, vote_count)
    VALUES (NEW.session_id, NEW.voted_for_id, 1);
  ELSE
    -- Sinon, incrémenter le compteur
    UPDATE participant_vote_results
  ...
  UPDATE participant_vote_results vr
  SET rank = ranks.rank
  FROM (
    SELECT id, ROW_NUMBER() OVER (ORDER BY vote_count DESC) as rank
    FROM participant_vote_results
    WHERE session_id = NEW.session_id AND deleted_at IS NULL
  ) ranks
  WHERE vr.id = ranks.id AND vr.session_id = NEW.session_id AND vr.deleted_at IS NULL;
...
BEGIN
  ...
  WHERE session_id = NEW.session_id AND voter_id = NEW.voter_id AND deleted_at IS NULL;
...
CREATE POLICY vote_settings_owner_policy ON vote_settings
  USING (
    ...
    AND vote_settings.deleted_at IS NULL
  )
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM sessions 
...
CREATE POLICY participant_votes_owner_view_policy ON participant_votes
  ...
  USING (
    ...
    AND participant_votes.deleted_at IS NULL
  );
...
-- Politique pour participant_vote_results: le propriétaire de la session peut voir tous les résultats
CREATE POLICY participant_vote_results_owner_policy ON participant_vote_results
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM sessions
      WHERE sessions.id = participant_vote_results.session_id 
      AND sessions.user_id = auth.uid()
      AND sessions.deleted_at IS NULL
    )
    AND participant_vote_results.deleted_at IS NULL
  );

-- Politique pour participant_vote_results: les participants peuvent voir les résultats publics
CREATE POLICY participant_vote_results_public_policy ON participant_vote_results
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM sessions
...
-- Fonction pour supprimer en douceur les données de vote d'une session
CREATE OR REPLACE FUNCTION soft_delete_participant_votes(session_id UUID)
RETURNS VOID AS $$
BEGIN
  -- Marquer tous les votes de cette session comme supprimés
  UPDATE participant_votes SET deleted_at = NOW() 
  WHERE session_id = session_id AND deleted_at IS NULL;
  
  -- Marquer les paramètres de vote comme supprimés
  UPDATE vote_settings SET deleted_at = NOW() 
  WHERE session_id = session_id AND deleted_at IS NULL;
  
  -- Marquer les résultats de vote comme supprimés
  UPDATE participant_vote_results SET deleted_at = NOW() 
  WHERE session_id = session_id AND deleted_at IS NULL;
END;
$$ LANGUAGE plpgsql;
